...
void DRW_Block_Record::parseCode(int code, dxfReader* reader) {
    ...
    case 102: // Looking, in particular, for BLKREFS to determine BLOCK appearances
        if (reader->getUtf8String() == "{BLKREFS")
        {
            // read the 331 code and the corresponding BLOCK reference 
            reader->readRec(&code);
            while (code == 331)
            {
                blks.push_back(reader->getUtf8String());
                reader->readRec(&code);
            }
        }
        else if (reader->getUtf8String() == "}") // done with BLKREFS or other app data
        break;
    default:
        // other codes we care about, e.g. 5 (handle) and 2 (name) are handled in TableEntry
        DRW_TableEntry::parseCode(code, reader);
        break;
    }
}
...
void DRW_Layer::parseCode(int code, dxfReader *reader){
    ...
    case 290:
        ...
        break;
    case 370:
        tmpLW = reader->getInt32(); // for debugging
        lWeight = DRW_LW_Conv::dxfInt2lineWidth(tmpLW);
