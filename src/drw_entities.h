...
*  @author Rallaz
*/
/* Bevara: we're going to preserve this - just for flexibility of writing the DXF out.
*   but we'll copy the insertion values up the to block class for later ease of translation
*    to SVG
*/
...
public:
    DRW_Block() {
        ...
        name = "*U0";
        blockHandle = "default";
        blkRecordHandle = 0xFFFF;
        insPoint.x = insPoint.y = insPoint.z = 0;
        insScale.x = insScale.y = insScale.z = 1.0;
        insAng = 0.0;
        colSpacing = rowSpacing = 0.0;
        colCount = rowCount = 1;
        color = -1;
        isEnd = false;
    }
...
public:
    ...
    std::string blockHandle;    /*!< code 5 - just because*/
    duint32 blkRecordHandle;    /*!< this is used to link the BLOCK_RECORD block list and the INSERT for each block*/
    DRW_Coord insPoint;         /*!< Insertion point from insertion entity, , code 10, 20 & 30 */
    DRW_Coord insScale;         /*!< insertion scale  41, 42, 43 */
    DRW_Coord insExtPt;         /*!< insertion extrustion pt from 210, 220, 230 */
    double insAng;              /*!< insertion angle, 50 */
    double colSpacing;          /*!< column spacing 44*/
    double rowSpacing;          /*!< row  spacing 45 */
    int colCount;               /*!< column count  70*/
    int rowCount;               /*!< row count  71*/
    int color;                  /*!< color  62*/
    std::list<DRW_Insert>insList;  /*! list of INSERT associated with this block*/
