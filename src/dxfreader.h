...
public:
    ...
    bool readStream(const char* inBuf, int inBufLen);
    bool readFile(std::ifstream* stream);
    bool readRec(int *code);
    bool peekRec(int* code);

    std::stringstream inData; // this will be our input data
...
protected:
    ...
    virtual bool peekCode(int* code) = 0; //return true if sucesful (not EOF)
...
public:
    ...
    virtual bool peekCode(int* code); // Bevara: not yet needed
...
public:
    ...
    virtual bool readCode(int *code);
    virtual bool peekCode(int* code);
    virtual bool readString(std::string *text);
    virtual bool readString();
    virtual bool readInt16();
    virtual bool readDouble();
...
class dxfReaderStream : public dxfReader {
public:
    dxfReaderStream(std::ifstream* stream) :dxfReader(stream) { skip = true; }
    virtual ~dxfReaderStream() {}
    virtual bool readCode(int* code);
    virtual bool peekCode(int* code);
    virtual bool readString(std::string* text);
    virtual bool readString();
